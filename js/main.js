class Translator {
    ...
    /**
     * To do:
     * - hit PHP file instead of API directly and hide API key - https://github.com/mettleworks/big-huge-thesaurus-client
     */
    async translate() {
    ...
    async translateWord(word) {
        const wordIsAlpha = /^[A-Za-z]+$/.test(word);
        const isCommonWord = this.isCommonWord(word);
        const translate = wordIsAlpha && word !== '' && !isCommonWord && this.apiAvailable;
        if (!translate) {
            return word;
        }
    ...
    async translateLine(line) {
        const inputWords = line.split(/([^A-Za-z'])/);
        let outputWords = [];
    ...
    /**
     * Returns synonym with capitalization modified to match the original word
     *
    ...
    matchCasing(synonym, originalWord) {
        ...
        if (originalWord === this.capitalizeFirstLetter(originalWord)) {
            return this.capitalizeFirstLetter(synonym);
        }
        return synonym;
    }
